...
# model loading utilities merged here
...
def init_on_device(device: torch.device, include_buffers: bool = False) -> Generator[None, None, None]:
    ...
    def register_empty_buffer(self, name, buffer):
        old_register_buffer(self, name, buffer)
...
from .mb_wrapper import mb_setup_args
...
class RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        ...
        # Ensure eps is a numeric type
        self.variance_epsilon = float(eps)
        # Validate parameter type
        if not isinstance(self.variance_epsilon, (int, float)):
            raise TypeError(f"variance_epsilon must be a numeric type, but received {type(self.variance_epsilon)}")
...
class NormAttentionNorm(nn.Module):
    def __init__(self, config: EvaConfig, layer_idx: int):
        super().__init__()
...
class DecoderLayer(nn.Module):
    def __init__(self, config: EvaConfig, layer_idx: int, **moe_kwargs):
        super().__init__()
...
class EvaPreTrainedModel(PreTrainedModel):
    ...
    def _set_update_state_dict(self, update_fn: Callable | None = None):
        ...
        for child in self._modules.values():
            ...
            if isinstance(child, EvaPreTrainedModel):
...
class EvaForCausalLM(EvaPreTrainedModel, GenerationMixin):
    _vocab_keys = ["model.embed_tokens.weight", "lm_head.weight"]

    def __init__(self, config: EvaConfig, meta_init: bool = False):
        super().__init__(config)
        self.model = EvaModel(config, meta_init=meta_init)
        self.vocab_size = config.vocab_size
